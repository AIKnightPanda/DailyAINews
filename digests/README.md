...
| 源 | 内容 | 典型条数 |
|---|---|---|
| AINews (smol.ai) | 当期条目，按 Twitter / Reddit 板块分组 | ~34 |
| Import AI | 当期引用的论文与项目 | ~12 |
| OpenAI News | 官方发布 | 1-3 |
| Google DeepMind | 官方发布 | 0-1 |
| The Rundown AI | 每日头条 | 1-2 |

**只取标题和链接，不抓正文。** 试过抓正文页：OpenAI 直接 403，DeepMind 抓下来的
15KB 文本大半是导航栏。RSS 是这几家唯一稳定的入口。

三条设计约束：

1. **URL 全程不经过模型。** 从 `fetch-extra.js` 提取到页面渲染，中间没有任何模型
   参与，所以**不可能出现编造或改写的链接**。模型也被明确要求不要把这些链接
   写进简报。
2. **零 token。** 链接墙由 `build-viewer.js` 直接从归档渲染，不进模型上下文。
3. **失败可见。** 某个源抓不到时，「链接」视图底部会列出是哪个源、什么错误，
   而不是静悄悄少一块。
