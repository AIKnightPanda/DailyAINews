...
├── groups.js              补充源的注册表、分组与顺序 —— 抓取、排版、页面上的
│                          「信息源」清单都读这一份
...
Import AI 只取 Jack Clark 用 **「Read more:」** 标出的链接 —— 那是他这一节真正推荐的东西；
标记之前的链接（往期期号、顺带提到的论文）是行文引用，不收。
早先按「是不是 jack-clark.net」判断，既会误伤他新发的文章，也修不了「自引用排在前、
真链接排在后」的段落（#431 就把 Togelius 那条挤掉了）。
...
```
16:33 UTC  Actions runner 抓六个源 → 提交 digests/extra-pending.json   （预抓，实际常晚 2–3 小时触发）
...
2026-09-11 又提前到 16:33——延迟涨到了 2.5–3 小时，09-07 那次预抓比 Routine 还晚到。
