...
1. 对齐分支并抓取（两件事放在同一条命令里）：
   ...
   - 若 status 是 skipped 且 digests/<issue>.md 已存在 —— 简报这一部分到此为止。
     不要读素材、不要写文件、不要提交，回复「feed 未更新，本次无需操作」，
     然后**接着做第二部分（灵感）**。
   - 其他情况（archived / refreshed，或该期 md 尚不存在）—— 继续下一步。

   **不管走哪个分支，都要看一眼 JSON 里的 `missing`。** 它列的是 digests/ 里
   缺着的期号。非空就原样列出来并发一条通知 —— **这几期不会自己回来**：
   第二天这里拿到的是届时上游最新的那一份，上游一恢复正常节奏，缺的那期
   就再也抓不到了。要补只能手工跑，而且得赶在上游下次出刊之前。
   （2026-09-06 就是这么差点丢掉的：上游那天没在惯常的 06:4x UTC 出刊，
   当晚 22:54Z 才补发，比 Routine 的 21:30Z 晚 84 分钟。早退判断完全正确，
   但当时没人知道少了一期。）
...
**`missing` 非空的话单独说一句，把期号列全。**
任何一步失败都要明说，不要粉饰。
```

## 灵感模块（第二条管线）

简报做完、push 成功之后，接着跑灵感那条线。它和简报**互不依赖** ——
简报早退（feed 没更新）时灵感照跑，灵感任何一步失败也不该回头影响简报。
...
简报那部分做完后（包括 feed 未更新而早退的情况），继续做这一段。
两条线互不依赖，这一段失败不要影响已经推送成功的简报。

7. 归档：

   node scripts/ideas-archive.js

   输出一行 JSON，读其中的 issue 和 status：
   - status 是 error —— 报告后结束这一部分
   - status 是 empty —— 当期没有新条目，跳到第 11 步（页面仍要重建）
   - status 是 nothing-new —— 正常：GH Actions 的 fetch-ideas.yml 这天
     已经抓过、归档过了，这里再跑只是确认没有漏网的，继续下一步
   - 其他情况继续
   同时看 failed 数组，有源抓失败要在最后如实报出源名和错误。
   还要看 `quiet` 数组：里面是**状态一直报 ok、却连续 7 期以上一条都没有**的源。
   这不算失败，所以不影响出刊，但十有八九是源断了（订阅退订了、页面改版了），
   照样在最后如实列出来。
   还有一个 `notices` 数组（比如「没配 Gmail 环境变量」这类）——**这是已知的
   非故障状态，不是当天出的问题**，只在跟用户的聊天汇报里提一句就行，
   **不要**写进 `ideas/<期号>.md` 或任何会进公开页面的地方（页面上只放
   「这个源今天有没有抓到数据」这类完成情况，配置类的提示留在这里的汇报里）。

8. 深挖（**这一步不能跳，但大多数时候会是空转**）：

   node scripts/ideas-deepen.js --issue=<issue>

   两条通道各自预筛（需求侧约 18 条、上新侧约 14 条），逐条抓回正文和评论。**评论是这套东西的命根子** ——
   「有没有竞品、需求真不真、是不是早就解决了」的答案通常只在评论里。
   2026-09-02 实测：一条「截图 30 秒后自动删除」的需求被选成当日第一，
   而它的第一条评论就是「iOS 本来就有这个功能」，发帖人自己回了「INCREDIBLE!」。

   **2026-09-06 起深挖已经搬去 GH Actions 先跑一遍**（那边没有这里沙箱的
   出网限制），已经有 `deep` 字段的条目会被跳过、不重新发请求——所以这一步
   多数时候几秒钟就完事，输出里 `ok`/`targets` 都很小是正常的，
   **不是抓漏了**。只有当天新冒出来、GH Actions 没赶上的少数条目才会在
   这里真正发请求；这些请求仍然可能撞上沙箱的出网策略（403 /
   `Blocked by egress policy`），撞上了如实报告，不要当成这一步失败了
   要重跑——重跑也大概率还是撞。reddit 要退避，正常情况这一步几秒到
...
9. 起一个 **model 为 haiku** 的子代理（**前台跑**，`run_in_background` 设为 false），任务是：
     ...
     任务 A 是给每条候选写中文标题和一句话说明（**每条都要有，不许留空**），
     跟点子/产品完全无关的纯讨论/生活话题/求职求助/自荐推广类还要把编号写进
     任务 B 的 exclude 字段（详见 ideas-style.md）。
     任务 B 是从中挑出真正值得做的（0 到 3 条是正常的，不要为了凑数硬选），
...
11. 建页并提交：
    ...
    node scripts/build-ideas-viewer.js && git add -A && \
      git commit -m "ideas: <issue>" && git push origin main

    没有变更时 commit 会失败，那是正常的，直接跳过。

结束汇报里补三句：灵感池内多少条、深挖成功几条、精选几条、有没有源抓失败、
有没有源被标成 quiet（连续多期 0 条）。
```

## ⚠️ 需要在 Routines 界面上改的配置

第 2 步和第 9 步都要起子代理，所以**工具清单里必须勾上 `Task`**。没勾上的话这一步会失败，
